...
😊 Вот! И это как раз тот момент, когда технические особенности API начинают влиять на архитектуру.

Ты абсолютно прав.

Для Realtime API обычно задаётся не пара языков, а инструкция вроде:

> *"Translate everything into English."*

Модель сама определяет язык входящего текста.

То есть фактически линия характеризуется **не направлением "из → в"**, а **целевым языком**.

---

## Тогда `TranslationLine` может выглядеть так:
...
Потому что источник — это "любой язык, отличный от целевого".

---

## Но...

Мне бы всё равно хотелось где-то сохранить знание о "сторонах моста".

Например:

```rust
pub struct TranslationBridge {
    left: TranslationLine,
    right: TranslationLine,
}
```
...
```text
left  → переводит всё в русский
right → переводит всё в английский
```

Обрати внимание: здесь уже нет `ru_to_en` и `en_to_ru`.

Есть **левая** и **правая** стороны моста.

Это очень похоже на настоящий мост.

---

## Тогда создание будет выглядеть красиво

Например:

```rust
let bridge = TranslationBridge::builder()
    .left(Language::Russian)
...
```text
 ...
 target = Russian               target = English
```

То есть:

* левая линия говорит: *"всё, что приходит сюда, переводи на русский"*;
* правая линия говорит: *"всё, что приходит сюда, переводи на английский"*.

Это полностью соответствует тому, как работает модель.

---

## И тут я увидел ещё одно преимущество

Такая модель вообще перестаёт зависеть от того, **кто говорит**.
...
* микрофон ↔ Zoom;
* Telegram ↔ Discord;
* SIP ↔ локальный микрофон.

Для `TranslationBridge` это уже не имеет значения. Есть две стороны, и каждая знает только свой целевой язык.

---

## Мне ещё нравится, что это хорошо масштабируется

Например, позже можно добавить свойства линии:

```rust
pub struct TranslationLine {
    target_language: Language,
    voice: Voice,
    audio_output: AudioSink,
    volume: f32,
}
```

И всё это относится именно к одной стороне моста.

---

### Мне кажется, мы постепенно нашли очень удачную терминологию:
...
* отражает реальную работу Realtime API;
* хорошо читается в коде;
* не привязана к конкретному провайдеру или модели;
* и, что немаловажно, легко объясняется новому разработчику буквально одной фразой: **"Мост состоит из двух линий, каждая переводит входящую речь в свой целевой язык."** Мне кажется, это одна из самых удачных архитектурных метафор, к которым мы пришли за последние дни.
